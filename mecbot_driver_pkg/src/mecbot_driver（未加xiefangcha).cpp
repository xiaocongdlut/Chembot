...
{
    double odom_vx = msg->linear.x;
    double odom_vy = msg->linear.y;
    double odom_vth = msg->angular.z;

    ros::Time odom_current_time = ros::Time::now();
    odom_dt = (odom_current_time - odom_last_time).toSec();

    double odom_delta_x = odom_vx * cos(odom_theta) * odom_dt;
    double odom_delta_y = odom_vx * sin(odom_theta) * odom_dt;
